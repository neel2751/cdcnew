...
const NewsletterPopup = () => {
  ...
  useEffect(() => {
    // Check if the "newsletterPopupDismissed" cookie is not present
    const isPopupDismissed = document.cookie
      .split(";")
      .some((item) => item.trim().startsWith("newsletterPopupDismissed="));

    // Set a timer to show the popup after 10 seconds if it's not dismissed
    const timer = setTimeout(() => {
      setIsVisible(!isPopupDismissed);
  ...
  const dismissPopup = () => {
    ...
    document.cookie =
      "newsletterPopupDismissed=true; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/";
